...
- Feature commit `62f15fc` and template commit `4b37ff8` include a 60-second deadline covering both the R2 response and streamed body, including abort/destruction on failure. A stalled verification download exposed the need for this bound; checksum migrations verify in bounded chunks before metadata writes.
- The isolated `62f15fc` release passed lint, typecheck, 142 unit tests (478 assertions), 13 isolated Mongo integration tests (48 assertions), production build and portfolio image checks. Its browser gate stopped on a mobile insertion test whose `End` key selected a wrapped-line boundary rather than the asserted paragraph boundary. Production was not deployed; the test setup is being corrected without weakening its insertion-position assertion.
- All 47 remaining demos now have consistent private controller/slot encryption and cron configuration (141 environment files), with private backups. The Brave's 31 reviewed legacy ownership records were applied successfully. ASAP and remaining demo checksum migrations are in progress; no excluded record is automatically trusted.
- Fleet release automation defaults to read-only review, stages exact committed feature files in isolated clones, requires every manifest gate, and deploys the validated commit through the inactive slot. Its committed scoped receipt preserves unrelated site customizations while recording full-template differences; normal full-baseline release checks remain the default. No production fleet deployment has run yet.
- All reviewed ownership migrations are complete and independently verified: 1,359 installed-demo records (including Popty Cara), 528 ASAP records and 31 The Brave records. All eight exclusions remain unchanged/untrusted. All 51 sites and The Brave's separate release controller have matching required newsletter configuration; no services were restarted by provisioning.
- The `f3e21ae` release stopped before deployment on a real delayed focus-restoration race after closing a media picker. Commit `ffc397d` preserves deliberate external focus. A deterministic regression failed against the old helper and passed after the fix; all six desktop/mobile upload, token and focus flows passed, with lint and typecheck green.
- Independent fleet runner review found and closed stale-dotenv inheritance, tracked operational-state mutation and current-main ancestry gaps. Seventeen runner tests (68 assertions), lint and typecheck pass. Root quality now runs the runner regression suite as an explicit gate. Deployment remains pending a complete passing exact-commit run.

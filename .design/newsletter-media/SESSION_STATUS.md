...
Checkpoint: 2026-09-19 18:08 UTC. **31/51 sites complete; 20 remain.** Current site: michaels-malmesbury; watcher: running; stage: all-manifest-quality-gates. Read ROLLOUT.json and fleet-release-final/post-deploy-status.json for newer results.
...
- Release queue: session `30256`, log `/tmp/newsletter-fleet-release-resume20.log`, reports `fleet-release-final/`. Explicit20-site selection starts at Michaels Malmesbury and includes only uncompleted sites. Every manifest gate and independent median guard runs before exact-main deployment and controller/mirror operations.
- Post-deploy watcher: session `54362`, log `/tmp/newsletter-fleet-post-deploy-resume20.log`, status `fleet-release-final/post-deploy-status.json`. Covers original47additionaldemos;27completedproofs. Root is sole cron writer through the locked helper. **Never pass --enable-dispatch for demos.**

...
Release checkpoint (2026-09-19 12:16 UTC): **14/51 sites are released and operationally verified; 37 remain.** Bush Farm passed its complete gates and post-deploy checks. Claire Lewis’s desktop Lighthouse stall is preserved for review; a fresh diagnostic server passed, and the unchanged candidate is running the entire quality suite again. The cause remains unproven. Blue Anchor’s port conflict and Bubbleton’s layout shift are resolved with fresh full validation; no baseline or threshold changes. Later progress is recorded in ROLLOUT.json and fleet-release-final/post-deploy-status.json.
...
- The managed rollout has 51 sites: hub, ASAP, The Brave and 48 installed demos. Four pilots and ten further demos are complete; 37 demos remain. Fleet quality releases are serial: measured examples are about 15–20 minutes per site; report actual progress per site.
...
Checked implementation tasks mean the shared code and stated local/pilot checks have passed. They do not mean all installed sites are running it. The four pilots and the first ten further demos are live and operationally verified: 14/51 complete. Popty and ASAP also passed actual storage round trips. All 51 sites' configuration and reviewed ownership migration are complete, and the complete shared source is staged. Tasks 8 and 9 retain the outstanding validation, release and post-deploy work for 37 demos.
...
- [x] **6. Delivery claims and lifecycle.** Add atomic claims, interrupted-delivery review states, cancellation/unschedule safeguards, frozen version checks and legacy campaign handling. Extend asset usage to newsletter references; source replacement/purge must not invalidate pinned/sent renditions. Add idempotent bounded cleanup and dry-run command with a scheduled vehicle. Test concurrent workers, provider ambiguity, crash windows, cleanup boundaries and missing snapshots.
  ...
  Evidence for tasks 2–6: shared unit tests, 13 isolated Mongo integration tests, desktop/mobile walkthrough flows, Puck regression checks, all four pilots' full gates/operational checks and actual Popty/ASAP storage checks. Cleanup is scheduled on all fourteen completed sites; installation on the remaining 37 demos remains part of task 9.
...
- [ ] **9. Complete source distribution and staged release.** Update the platform template and explicit sync allowlist for the full editor/API/model/service/serializer/tests/cleanup set; carry the complete Asset model bookkeeping fields to ASAP/The Brave. Apply reviewed scoped diffs preserving site-specific changes. Commit only this task's changes, preserve the user's unrelated dirty files, validate the exact committed release tree, then deploy through inactive slots. Pilot hub + one installed demo, then ASAP/The Brave, then serial batches of remaining verified targets. Record commits, gates, smoke checks and rollback revisions. Publish the already-tested histories to canonical remotes. Never silently fall back to old code for queued media manifests.
  ...
  Status: Template `87c7e45`, all 98 shared files, explicit allowlist/installer changes, all 51 configurations and all 1,918 reviewed ownership records are complete. Eight excluded records remain untrusted. All four pilots and ten further demos have completed release and nginx/scheduler/cleanup verification: 14/51 complete, 37 remaining. Hub, ASAP and The Brave are mirrored; missing demo canonical remotes are recorded as skips. Queue session `54090`, log `/tmp/newsletter-fleet-release-resume37.log`, starts at Claire Lewis and covers exactly the 37 uncompleted sites. Earlier failures are preserved under `fleet-release-history/`. Separate post-deploy watcher `35375` verifies each healthy release and installs read-only demo schedules plus daily cleanup. Do not mark rollout complete yet.

...
function randomAlphaNumeric(length: number) {
  ...
  for (let i = 0; i < length; i += 1) {
    result += chars[randomInt(0, chars.length - 1)] || 'a';
  }
  return result;
}

function randomWords(min: number, max: number) {
  const count = randomInt(min, max);
  const tokens = Array.from({ length: count }, () => RANDOM_WORDS[randomInt(0, RANDOM_WORDS.length - 1)] || 'text');
  return tokens.join(' ');
}

function randomSentence() {
  const text = randomWords(4, 7);
...
const fakeStreamText = ({
  ...
  return new ReadableStream({
    async start(controller) {
      ...
      const messageId = `msg_${randomAlphaNumeric(40)}`;
      ...
      return [
        {
          ...
          texts: `{"id":"${nanoid()}","data":{"blockId":"${block.id}","comment":"${randomSentence()}","content":"${content}"},"type":"data-comment"}`,
        },
      ];
    })

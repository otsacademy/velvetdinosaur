...
const NEWS_EDITOR_SLASH_EVENT = 'news-editor-open-slash-menu';
      ...
      {!isInTable && (
        <Gutter>
          ...
          >
            ...
            >
              <Button
                className="absolute right-0 h-6 w-6 rounded-md p-0 text-[rgb(180,180,180)] transition-colors duration-150 ease-out hover:bg-muted/70 hover:text-[rgb(100,100,100)]"
...
function Gutter({
  ...
  return (
    <div
      ...
      className={cn(
        'slate-gutterLeft',
        'absolute top-0 left-[-48px] z-50 flex h-full cursor-text opacity-0 transition-opacity duration-150',
        getPluginByType(editor, element.type)?.node.isContainer
...
const DraggableInsertHandle = () => {
  ...
  return (
    <Button
      className="size-6 shrink-0 rounded-md p-1 text-[rgb(180,180,180)] transition-colors duration-150 ease-out hover:bg-muted/70 hover:text-[rgb(100,100,100)]"
      onClick={(event) => {
        event.stopPropagation();
        event.preventDefault();

        const at = editor.api.findPath(element);
        const handledByCustomMenu =
          typeof window !== 'undefined' &&
          !window.dispatchEvent(
            new CustomEvent(NEWS_EDITOR_SLASH_EVENT, {
              cancelable: true,
              detail: {
                at,
                insertAbove: event.altKey,
              },
            })
          );

        triggerComboboxNextBlock(editor, handledByCustomMenu ? '' : '/', at, event.altKey);
      }}
      onMouseDown={() => {
        editor.tf.focus();

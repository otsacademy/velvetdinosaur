...
import { SUPPORT_TICKET_STATUSES, SUPPORT_WAITING_ON, supportWaitingOnLabel } from '@/lib/support/constants';
import {
  formatDateTime,
...
import {
  ...
  statusDescription,
  waitingOnBadgeClass
} from '@/components/edit/support/support-ticket-detail.helpers';
          ...
          <>
              ...
              <CardContent className="space-y-4 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className={priorityBadgeClass(thread.ticket.priority)}>
                      {thread.ticket.priorityLabel}
                    </Badge>
                    <HoverCard openDelay={150} closeDelay={100}>
                      <HoverCardTrigger asChild>
                        <Badge variant="outline" className={statusBadgeClass(thread.ticket.status)}>
                          {thread.ticket.statusLabel}
                        </Badge>
                      </HoverCardTrigger>
                      <HoverCardContent className="w-64 text-sm" sideOffset={8}>
                        <p className="font-medium">{thread.ticket.statusLabel}</p>
                        <p className="mt-1 text-muted-foreground">{statusDescription(thread.ticket.status)}</p>
                      </HoverCardContent>
                    </HoverCard>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <Badge variant="outline" className={waitingOnBadgeClass(thread.ticket.waitingOn)}>
                ...
                <CardContent className="space-y-3 p-4">
                  ...
                  <div className="flex items-start gap-3">
                    <Avatar className="h-9 w-9 border border-sidebar-primary/35">
                      <AvatarImage src="/dinosaur.webp" alt="Velvet Dinosaur" className="object-cover" />
                  ...
                  <p className="text-xs text-muted-foreground">
                    Plain text is sent to the customer portal. Include links to files if you need to share attachments.
                  </p>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Ratings are enabled when a request is in Resolved or Closed status.
                </p>
              </div>
              ...
              <div className="rounded-md border bg-muted/20 p-3 text-sm">
                <p className="flex items-center gap-1">
                  ...
                  <span className="font-medium">
                    {thread.ticket.satisfactionRating ? `${thread.ticket.satisfactionRating}/5` : 'Not submitted'}
                  </span>
                ...
                {thread.ticket.satisfactionComment ? (
                  <p className="mt-1 text-muted-foreground">{thread.ticket.satisfactionComment}</p>
                ) : null}
              </div>

import type { SupportTicketThread, SupportWaitingOn } from '@/components/edit/support/support-workspace.shared';
...
export function statusDescription(status: string) {
  const descriptions: Record<string, string> = {
    ...
    resolved: 'Completed and awaiting any final feedback.',
...
export function resolveMessageName(message: SupportTicketThread['messages'][number], type: MessageType) {
  ...
  return message.authorEmail || 'Velvet Dinosaur';
}
...
export function resolveMessageAvatar(message: SupportTicketThread['messages'][number], type: MessageType) {
  ...
  if (type === 'staff') return '/dinosaur.webp';
  return '';
}
...
export function messageTone(type: MessageType) {
  ...
  return {
    bubble:
      'rounded-2xl rounded-tr-sm border border-sidebar-primary/35 bg-sidebar-primary text-sidebar-primary-foreground shadow-sm',
    badge: 'border-sidebar-primary/35 bg-sidebar-primary/15 text-sidebar-primary',
    label: 'Velvet Dinosaur'
  };
}

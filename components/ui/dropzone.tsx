    ...
    <div className={cn('flex flex-col items-center justify-center', className)}>
      ...
      <p className="my-2 w-full truncate text-sm font-medium">
        {src.length > maxLabelItems
          ? `${new Intl.ListFormat('en').format(src.slice(0, maxLabelItems).map((file) => file.name))} and ${
              src.length - maxLabelItems
            } more`
...
export const DropzoneEmptyState = ({ children, className }: DropzoneEmptyStateProps) => {
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center', className)}>
      ...
      {caption ? <p className="text-wrap text-xs text-muted-foreground">{caption}.</p> : null}
    </div>
  );
};

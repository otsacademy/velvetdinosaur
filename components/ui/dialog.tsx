  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          'fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 rounded-[var(--vd-radius)] border border-[var(--vd-border)] bg-[var(--vd-bg)] p-6 shadow-lg duration-200 data-[state=closed]:animate-out data-[state=open]:animate-in data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95',
          className
        )}
        {...props}
      >
        {children}
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-[var(--vd-radius)] p-1 opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--vd-ring)] focus:ring-offset-2">
          <X className="h-4 w-4" />
...
export function DialogTitle({
  ...
  return (
    <DialogPrimitive.Title
      ...
      className={cn('text-lg font-semibold text-[var(--vd-fg)]', className)}
      {...props}
    />
  );
}

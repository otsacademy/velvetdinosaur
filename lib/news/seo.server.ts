import { extractPlainTextFromPlate, normalizePlateValue } from '@/lib/news-plate-transform'
...
export type NewsSEOTargetArticle = {
  title?: string | null
  desc?: string | null
  content?: unknown
...
function deriveArticleBody(article: NewsSEOTargetArticle) {
...
export function shouldGenerateSEOForArticle(
  ...
  const hasExistingTitle = trimAndNormalize(article.seoTitle || '')
  const hasExistingDescription = trimAndNormalize(article.seoDescription || '')
...
export async function computeSEOForArticle(
  ...
  const articleTitle = trimAndNormalize(article.title || '')
  if (!articleTitle) {
    return null
  }
...
export function buildManualSeoPatch(article: NewsSEOTargetArticle, next: { title: string; description: string }) {
...
export async function persistAutoGeneratedSEO(
  ...
  const result = await NewsArticle.findOneAndUpdate(filter, { $set: update }, { new: false }).exec()

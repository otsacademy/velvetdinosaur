...
export default function DemoNewsHostLayout({ children }: { children: React.ReactNode }) {
  return (
    <Suspense fallback={<div className="p-6 text-sm text-[var(--vd-muted-fg)]">Loading editor...</div>}>
      <DemoNewsHostLayoutContent>{children}</DemoNewsHostLayoutContent>
    </Suspense>
  );
}

async function DemoNewsHostLayoutContent({ children }: { children: React.ReactNode }) {
  await connection();
